...
# substitute your application name for instances of kcts

# Variables
!define kcts_DESCRIPTION "${kcts_NAME} ${kcts_VERSION}"
!define kcts_STRING "${kcts_NAME}-${kcts_VERSION}"

!define PRODUCT_BINARY "${kcts_BINARY}"
!define PRODUCT_NAME "${kcts_NAME}"
!define PRODUCT_VERSION "${kcts_VERSION}"
!define PRODUCT_STRING "${kcts_STRING}"
!define PRODUCT_DESCRIPTION "${kcts_DESCRIPTION}"
...
VIAddVersionKey FileDescription "${kcts_DESCRIPTION} installer"
...
Section -install
    ...
    WriteRegStr HKLM "${REG_UNINSTALL_PATH}" "Publisher" "kcts developers"
...
Section "Start Menu Shortcuts"
	...
	CreateShortCut "${SM_PATH}\${kcts_NAME}.lnk" "$INSTDIR\${kcts_BINARY}" "" "$INSTDIR\${kcts_BINARY}" 0
#	CreateShortCut "${SM_PATH}\${kcts_NAME} Beginners' Guide.lnk" "${GUIDE_URL}"
#	CreateShortCut "${SM_PATH}\${kcts_NAME} Documentation.lnk" "${kcts_DOCS_URL}"
    CreateShortCut "${SM_PATH}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
SectionEnd

Section "Desktop Shortcuts"
	CreateShortCut "$DESKTOP\${kcts_DESCRIPTION}.lnk" "$INSTDIR\${kcts_BINARY}" "" \
		"$INSTDIR\${kcts_BINARY}" 0
SectionEnd

# This is unselected by default
Section /o "Quick Launch Shortcuts"
	CreateShortCut "$QUICKLAUNCH\${kcts_DESCRIPTION}}.lnk" "$INSTDIR\${kcts_BINARY}" "" \
		"$INSTDIR\${kcts_BINARY}" 0
SectionEnd
...
# Remove files and uninstaller
	Delete /REBOOTOK $INSTDIR\${kcts_BINARY}
    Delete /REBOOTOK $INSTDIR\uninstall.exe

# Remove shortcuts, if any
    Delete "${SM_PATH}\*.*"
	Delete "$DESKTOP\${kcts_DESCRIPTION}.lnk"
	Delete "$QUICKLAUNCH\${kcts_DESCRIPTION}.lnk"
