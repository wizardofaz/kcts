AC_DEFUN([AC_kcts_XMLRPC_CONFIG], [
  ac_cv_xmlrpc=no
  ...
  if test "x$XMLRPC_CFLAGS" != "x" && test "x$XMLRPC_LIBS" != "x"; then
      ...
      if test "x$XMLRPC_C_CONFIG" != "xno" && $XMLRPC_C_CONFIG c++2 client; then
          ac_cv_xmlrpc=yes

          test "x$XMLRPC_CFLAGS" = "x" && XMLRPC_CFLAGS=`$XMLRPC_C_CONFIG c++2 client --cflags`
          if test "x$XMLRPC_LIBS" = "x"; then
              XMLRPC_LIBS=`$XMLRPC_C_CONFIG c++2 client --ldadd`
